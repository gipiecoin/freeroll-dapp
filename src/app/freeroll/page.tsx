...
const FREEROLL_CONTRACT_ADDRESS = "0x792d1336f3BA157C3B088B51a8C18BD9565a1145";
...
type RollHistoryEntry = { roll: number; timestamp: number }; 
...
export default function Freeroll() {
  ...
  const canClaim = connected && !isLoading && !isProcessingTx && parseFloat(pendingRewards) > 0;
  ...
  const updateRollHistory = useCallback(async () => {
    ...
    const [newRoll] = await freerollContract.getUserRollInfo(address); // Hanya ambil roll, abaikan pendingRewards
    const newEntry = { roll: Number(newRoll), timestamp: Date.now() };
    ...
    history = history.slice(0, 3); // Batasi hingga 3 entri
  ...
  const checkStatus = useCallback(async () => {
    ...
    try {
      ...
      const [rollNumber, pendingRewardsValue] = userInfo;
      setLastRoll(Number(rollNumber) > 0 ? Number(rollNumber) : null);
      setPendingRewards(Number(rollNumber) > 0 ? ethers.formatUnits(pendingRewardsValue, 18) : "0");
    } catch (err: unknown) {
  ...
  const handleRoll = async () => {
    ...
    if (receipt) {
      const [newRoll, newPendingRewards] = await contract.getUserRollInfo(await signer.getAddress());
      stopRollAnimation(Number(newRoll));
      setPendingRewards(ethers.formatUnits(newPendingRewards, 18));
  ...
  const handleClaim = async () => {
    ...
    const signer = await provider.getSigner();
    const contract = new ethers.Contract(FREEROLL_CONTRACT_ADDRESS, FreerollABI, signer);
    await handleTransaction(contract.claimReward(), "Rewards claimed successfully!", "Claim Failed.");
    await checkStatus();
  };
  ...
  if (lastRoll !== null) {
    const winAmount = parseFloat(pendingRewards) > 0 ? ` & win ${pendingRewards} GIPIE` : "";
    const tweetText = encodeURIComponent(
      `I rolled ${lastRoll.toString().padStart(5, "0")}${winAmount} on @gipiecoin Freeroll! 🍀 Try your luck #binance #bsc #dapp #airdrop #airdrops #btc #eth #roll👉🏻:`
    );
    const tweetUrl = `https://twitter.com/intent/tweet?text=${tweetText}&url=${encodeURIComponent(DAPPS_BASE_URL + "/freeroll")}`;
    window.open(tweetUrl, "_blank");
  } else {
    showMessage("You need to roll first to share!");
  }
}, [lastRoll, pendingRewards, showMessage]);
        ...
        <div className="bg-gray-800 bg-opacity-60 backdrop-blur-lg p-6 sm:p-8 rounded-2xl shadow-2xl w-full max-w-lg text-white border border-gray-700">
          ...
          <div className="mb-6 border border-gray-700 rounded-lg overflow-hidden bg-gray-900/60">
            <div className="p-4">
              <table className="w-full text-sm text-left text-gray-300">
                ...
                <tbody>
                  {[
                    { range: "10000", payout: "100", jackpot: true },
                    { range: "9998 - 9999", payout: "10" },
                    { range: "9994 - 9997", payout: "1" },
                    { range: "9986 - 9993", payout: "0.1" },
                    { range: "9886 - 9985", payout: "0.05" },
                    { range: "0 - 9885", payout: "0.01" },
                  ].map((tier) => (
          ...
          {lastRoll !== null && (
            <div className="mb-6 p-5 bg-gradient-to-r from-purple-800 via-indigo-800 to-purple-800 rounded-lg shadow-lg text-center border border-purple-600">
              ...
              <p className="text-lg font-semibold text-gray-400 mb-3">Roll to add to your pending rewards!</p>
              <button
                onClick={handleShareToX}
                className="mt-2 px-5 py-2 rounded-full text-white font-bold text-sm transition-all bg-sky-500 hover:bg-sky-600"
              >
                Share on 𝕏
              </button>
            </div>
          ...
          {rollHistory.length > 0 && (
            <div className="mb-6 border border-gray-700 rounded-lg overflow-hidden">
              <button
                ...
                className="w-full p-4 bg-gray-800 bg-opacity-80 hover:bg-opacity-90 flex justify-between items-center text-left"
              >
                <span className="font-semibold text-teal-300">Your Recent Rolls</span>
                <svg
                  className={`h-5 w-5 text-gray-400 transition-transform ${openSection === "history" ? "rotate-180" : ""}`}
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                >
                    ...
                    {rollHistory.map((entry) => (
                      <div key={entry.timestamp} className="flex justify-between items-center text-sm p-2 bg-gray-800 rounded-md">
                        <span className="font-mono text-gray-400">
                          Rolled: <span className="font-bold text-white">{entry.roll.toString().padStart(5, "0")}</span>
                        </span>
                        {/* Tidak ada payout, hanya nomor roll */}
                      </div>
                    ))}
                  </div>
                </div>
          ...
          <div className="mb-6 bg-gray-900/50 p-4 rounded-lg shadow-lg">
            ...
            <div className="grid grid-cols-2 gap-4">
              <button
                onClick={handleRoll}
                disabled={!canRoll}
                className="py-3 px-4 rounded-lg font-bold text-xl transition-all bg-gradient-to-r from-blue-500 to-purple-600 text-white enabled:hover:scale-105 disabled:cursor-not-allowed disabled:opacity-50"
              >
              ...
              <button
                onClick={handleClaim}
                disabled={!canClaim}
                className="py-3 px-4 rounded-lg font-bold text-xl transition-all bg-gradient-to-r from-green-500 to-teal-500 text-white enabled:hover:scale-105 disabled:cursor-not-allowed disabled:opacity-50"
              >
                {isProcessingTx ? "Processing..." : parseFloat(pendingRewards) > 0 ? `Claim ${pendingRewards} GIPIE` : "No Rewards"}
              </button>
            </div>
          </div>
                ...
                <div className="mb-4">
                  ...
                  <div className="w-full bg-gray-700 rounded-full h-2.5">
                    <div
                      ...
                      style={{ width: `${Math.min(dailyRollCount, 10) / 10 * 100}%` }}
                    ></div>
                  </div>
                  {resetMessage && (
                  ...
                  {isBonusEligible && (
                    <button
                      ...
                      className="mt-4 w-full py-3 px-4 rounded-lg font-bold text-xl transition-all bg-gradient-to-r from-purple-500 to-indigo-600 text-white hover:scale-105 disabled:cursor-not-allowed disabled:opacity-50 animate-fade-in"
